...
from ndfc import NDFC, SimpleLog
...
class SimpleLogger:
    ...
    def init_logger(self, level):
        ...
        if not isinstance(level, int):
            print(f"Exiting. Invalid log level: {level}")
            sys.exit(1)
        fstring = "%(asctime)s %(levelname)s: %(message)s"
        logging.basicConfig(format=fstring, level=level, force=True)
...
class NDFC:
    ...
    logger = SimpleLogger('example_log', 'INFO', 'DEBUG')
    ndfc = NDFC()
    ndfc.username = "my_username"
    ndfc.password = "my_password"
    ndfc.ip4 = "10.1.1.1"
    ...
    def _init_default_logger(self):
        ...
        """
        self.logger = SimpleLogger("debug")
    ...
    def url_base(self):
        ...
        if self.ip4 is None:
            self.logger.error(
                "Exit. Set instance.ip4 before calling NDFC() url properties."
            )
